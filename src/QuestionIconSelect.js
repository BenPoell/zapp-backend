 ...
 */
class QuestionIconSelect extends Component {
    state = {
  ...
  render() {
    return (
    <div>
      <Subheader style={formStyle.questionSelectFieldSubHeader}>Question Icon:</Subheader>
      <div style={formStyle.questionAvatarContainerDiv}>
        <QuestionAvatar content={(avatarIcons[`${this.state.value}`]) ? avatarIcons[`${this.state.value}`] : this.props.name.split(' ')[0].substr(0,1).toUpperCase() + (this.props.name.split(' ').length >= 2 ? this.props.name.split(' ')[1].substr(0,1).toUpperCase() : '') }/>
      </div>
      <DropDownMenu
          value={this.state.value}
          onChange={this.handleChange}
          style={formStyle.questionIconDropDownMenu}
          underlineStyle={formStyle.questionIconDropDownMenuUnderLineStyle}
          maxHeight={200}
        >
        {items}
      </DropDownMenu>
      <Divider style={formStyle.questionIconSelectDivider}/>
    </div>
    );
  }
}
